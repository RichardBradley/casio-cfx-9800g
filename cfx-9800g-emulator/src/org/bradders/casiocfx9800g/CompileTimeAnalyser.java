...
public class CompileTimeAnalyser extends DepthFirstAdapter
{
   ...
   public CompileTimeAnalyser(CompiledFile file)
   {
      this.file = file;
   }
   ...
   public void outStart(Start node)
   {
      ...
      for (AGotoStatement gotoStatement : gotos) {
         ...
         if (!file.statementIdxByLabelNumber.containsKey(labelNumber)) {
   ...
   {
      file.statements.add(node.getStatement());
   ...
   public void caseALabelStatement(ALabelStatement statement)
   {
      ...
      int statementIdx = file.statements.indexOf(statement);
      if (statementIdx < 0) {
         throw new RuntimeException("Could not find parent statement: " + statement);
      }
      Integer previousStatementIdx = file.statementIdxByLabelNumber.put(labelNumber, statementIdx);
      if (null != previousStatementIdx) {
         throw new RuntimeException(String.format(
               "Duplicate label: '%s' is used at %s and %s",
               labelNumber,
               Printer.nodeToString(file.statements.get(previousStatementIdx)),
